...
import { resetTaskList } from '../../redux/tasks/actions'
...
export default connect(state => {
	...
	constructor(props) {
		super(props);
		for (let columnName of this.props.tasks.taskBoard.columnOrder) {
			let columnData = this.props.tasks.taskBoard.columns[columnName];
			let targetName = "taskBoard:" + columnName;
			registerDropContainer(targetName, (index) => {
				return this.props.tasks.taskBoard.columns[columnName].tasks[index];
			}, (fromIndex, toIndex) => {
				var originalList = this.props.tasks.taskBoard.columns[columnName].tasks;
            ...
            <div>
        		...
        		<div className="taskboard-board">
                    <div className="taskboard-board-inner">
        			    {this.props.tasks.taskBoard.columnOrder.map((columnName, index) => {
        					let targetName = "taskBoard:" + columnName;
        					let columnData = this.props.tasks.taskBoard.columns[columnName];
        			    	return (
        						    ...
        						    <Droppable key={columnName} droppableId={targetName}>
        				                {(dropProvided, dropSnapshot) => (
        				  	                <div ref={dropProvided.innerRef} className="taskboard-board-column-list">
        				  	                	{columnData.tasks.map((taskId, index) => {
                                                    let task = this.props.tasks.tasks[taskId];
                                                    return (
